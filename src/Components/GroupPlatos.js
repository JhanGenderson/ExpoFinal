...
export default function GroupPlatos({
  ...
  return (
    <div className="container" >
      <div>
        <h2 className="fw-bold text-center p-1">{categoriaPlato}</h2>
      </div>
      ...
      <div className="row mt-3">
        {plato
          ...
          .map((plat, i) => (
            <div
              className="col-12 col-lg-3 col-md-6 col-sm-6 align-self-center"
              key={i}
            >
              <Link
                style={{ textDecoration: "none", color: "black", height:'370px' }}
                className="card mb-4 border border-2 border-success"
                to={`/detalle/${plat.id}`}
              >
